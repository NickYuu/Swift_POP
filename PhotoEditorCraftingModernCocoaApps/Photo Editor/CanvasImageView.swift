...
class CanvasImageView: NSView {
    ...
    fileprivate func getEditMode() -> EditMode {
    ...
    fileprivate func sendImageChanged() {
        if let delegate = delegate {
    ...
    fileprivate func trackForMove(event mouseDownEvent: NSEvent) {
    ...
    // "draw" on the image
    fileprivate func trackForDraw(event mouseDownEvent: NSEvent) {
